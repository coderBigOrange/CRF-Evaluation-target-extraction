...
from nltk import *
import re
f1=open('upper_lap.txt','r',encoding='UTF-8')
f2=open('lable_lap.txt','w',encoding='UTF-8')
f3=open('dictionary.txt','r',encoding='UTF-8')
text=f1.read()
dictionary=f3.read()
...
def get_features(i):
    ...
    if type(i)==tuple:
        ...
        if((i[0]+' ').lower() in words):
            s=i[0].lower()+' '+pos+' '+'Y'+' '+'OP'+' '+'C'
        else:
            if (i[0].isupper()==True):
                s=i[0].lower()+' '+pos+' '+'N'+' '+'OP'+' '+'T'
            else:
                s=i[0].lower()+' '+pos+' '+'N'+' '+'OP'+' '+'O'
        f2.write(s+'\n')
    else:
        lab=i.label()
        for j in i:
            if type(j)==tuple:
                ...
                if((j[0]+' ').lower() in words):
                    s=j[0].lower()+' '+pos+' '+'Y'+' '+lab+' '+'C'
                else:
                    if (j[0].isupper()==True):
                        s=j[0].lower()+' '+pos+' '+'N'+' '+lab+' '+'T'
                    else:
                        s=j[0].lower()+' '+pos+' '+'N'+' '+lab+' '+'O'
                f2.write(s+'\n')
            else:
                get_features(j)
